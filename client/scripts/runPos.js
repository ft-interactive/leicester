...
function runPos(frame,data){
	...
	var highlight = ['Leicester City-2015','Sheffield Wednesday-1928','Ipswich Town-1980'];
	...
	var colours = d3.scale.ordinal()
		.domain(highlight)
		.range(['#af516c','#0091a7','#5ba829','#87cbf2','#b4bf2c','#ccc2c2','#3267b4','#f3abc8','#b07979']);

	var textColours = colours.copy();
	textColours.range(['#9e2f50','#008094','#5ba928','#69c0f2','#a7b224','#bfb0b0','#3267b4','#f197bc','#a96565']);
	...
	frames.each(function(d,i){
		...
		var minPos = d.values[0].values[0].natPos;

		var chartFrame = outer.append('div');

		// chartFrame.append('span').html(d.key.replace(/-/g,' '));

		chartFrame.append('span').html(d.key.replace(/-.*/g,'') + ' ' + d.values[0].values[0].date.getFullYear() + '-' + (d.values[1].values[0].Season+1).toString().substr(2,3));
		...
		.html('Days since leaving foot of table');
		...
		if(i==0){
			...
			.tspans(['&larr;','365-day','run from','foot of table','&larr;'])
			.attr({
				x:x(365)-4,
				// y:y(+plots.select('.y .tick:nth-of-type(1) text').html())-18
			});

			var xt2 = plots.append('text.x.axis.title')
			.attr({
				x:x(365)+4,
				y:y(minPos-3)-57
			})
			...
			.tspans(['&rarr;','End of','second','season','&rarr;'])
			.attr({
				x:x(365)+4,
				// y:y(+plots.select('.y .tick:nth-of-type(1) text').html())-18
			});
		...
		var endRect = plots.insert('rect','.axis')
			.attr({
				x:x(365),
				width:x.range()[1]-x(365)+(M.R*2),
				y:y(1),
				height:y(+plots.select('.y .tick:nth-of-type(1) text').html())-y(1),
				fill:'#fff1e0'
			});

		var relRect = plots.insert('rect','.axis')
			.attr({
				x:x(0),
				width:x.range()[1]-x(0)+(M.R*2),
				y:y(minPos-[2,1,1][i]),
				height:y([2,1,1][i])-y(0),
				fill:'#ffe3c2'
			});
		...
		var annos = [
			[
					{	
						key:highlight[0],
						point:[35,13.8],
						text:'Escaped relegation under Pearson in first season',
						textPoint:[20,-40],
						anchor:'start',
						side:-1
					},
			...
			[
					{	
						key:highlight[2],
						point:[75,13],
						text:'Climbed from bottom to third in first season',
						textPoint:[25,35],
						anchor:'start',
						side:1
					},
					{	
						key:highlight[2],
						point:[573,2],
						text:'Narrowly missed out on title win in second season',
						textPoint:[-20,20],
						anchor:'end',
						side:-1
					}
			...
			[
					{	
						key:highlight[1],
						point:[25,20],
						text:'Avoided relegation with three games to play',
						textPoint:[30,-25],
						anchor:'start',
						side:1
					},
					{	
						key:highlight[1],
						point:[395,1],
						text:'Only other team ever to win title after climbing off the bottom',
						textPoint:[-25,20],
						anchor:'end',
						side:-1
					}
 ...
 //    function drawChart(){
	...
	// 				var cx2 = (x1) + (((-y1 + y(d.point[1])))*(length*2/3))/(length);
	// 				var cy2 = (y1) + (((x1 - x(d.point[0]))))*(length*2/3)/(length);
	// 				var cx1 = (x2) + (((-y2 + y(d.point[1])))*(length/3))/(length);
	// 				var cy1 = (y2) + (((x2 - x(d.point[0]))))*(length/3)/(length);
	// 				d.path = 'M' + (x(d.textPoint[0])+d.textX) + ',' + (y(d.textPoint[1])+d.textY) + 'C' + cx1 + ',' + cy1 + ',' + cx2 + ',' + cy2 + ',' + (x(d.point[0])-d.pointX) + ',' + (y(d.point[1])-d.textY);
